...
export class TasksComponent implements OnInit {
	...
	async ngOnInit(): Promise<void> {
		...
		this.showInactiveTasks = (await this.databaseService.getSetting(SettingKey.Tasks_ShowInactive)) ?? false;
		this.sortOrder = (await this.databaseService.getSetting(SettingKey.Tasks_SortOrder)) ?? SortOrder.AZ;
	...
	getTagsForTask(task: Task) {
		return this.tags.filter(t => task.tags.includes(t.id)).orderBy(t => t.name);
	}

	async toggleShowInactiveTasks() {
		this.showInactiveTasks = !this.showInactiveTasks;
		await this.databaseService.updateSetting(SettingKey.Tasks_ShowInactive, this.showInactiveTasks);
	}
	...
	async sortOrderChanged(sortOrder: SortOrder) {
		...
		this.sortTasks();
		await this.databaseService.updateSetting(SettingKey.Tasks_SortOrder, sortOrder);
	}
