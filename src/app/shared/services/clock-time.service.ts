...
export class ClockTimeService {
    ...
    async addClockTime(clockTime: ClockTime): Promise<ClockTime> {
        ...
        this.clockTimeSavedSource.next(
            new ClockTimeEvent(clockTime, ClockTimeEventType.Added)
        );
        return clockTime;
    }
    ...
    async updateClockTime(clockTime: ClockTime): Promise<ClockTime> {
        ...
        this.clockTimeSavedSource.next(
            new ClockTimeEvent(clockTime, ClockTimeEventType.Updated)
        );
        return clockTime;
    }
    ...
    async getClockTimesInDateRange(
        minDate: Date,
        maxDate: Date
    ): Promise<ClockTime[]> {
    ...
    async getClockTimeByIds(ids: number[]): Promise<ClockTime[]> {
        return await this.databaseService.getClockTimesByFilter((t) =>
            ids.includes(t.id)
        );
    }
}
