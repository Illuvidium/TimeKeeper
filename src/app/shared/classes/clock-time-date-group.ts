...
export class ClockTimeDateGroup {
    ...
    updateStats() {
        ...
        const minTime: number = this.momentDate
            .startOf('day')
            .toDate()
            .getTime();
        const maxTime: number = Math.min(
            this.momentDate.endOf('day').toDate().getTime() + 1,
            new Date().getTime()
        );
...
export function calculateElapsedForDate(clockTime: ClockTime, date: Date) {
    ...
    const maxTime: number = Math.min(
        momentDate.endOf('day').toDate().getTime() + 1,
        new Date().getTime()
    );
...
export function calculateElapsedMs(
    ...
    const ctFinish = clockTime.finish
        ? new Date(clockTime.finish).getTime()
        : maxTime;
...
export function getStartFinishTimeString(
    clockTime: ClockTime,
    date: Date
): string {
    const momentDate = moment(date);
    const momentStart = moment(clockTime?.start);
    const startTime = momentStart.isBefore(momentDate, 'day')
        ? '00:00'
        : momentStart.locale('nl').format('LT');

    if (!clockTime.finish) return `${startTime} -`;

    const momentFinish = moment(clockTime?.finish);
    const finishTime = momentFinish.isAfter(momentDate, 'day')
        ? '23:59'
        : momentFinish.locale('nl').format('LT');
