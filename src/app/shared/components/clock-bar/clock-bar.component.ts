...
export class ClockBarComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		this.tagSavedSubscription = this.tagService.tagSaved.subscribe(event => {
			switch (event.type) {
				case TagEventType.Added:
					this.tags.push(event.tag);
					break;
				case TagEventType.Updated:
					this.tags = this.tags.filter(t => t.id !== event.tag.id);
					this.tags.push(event.tag);
					break;
			}
		...
		this.taskSavedSubscription = this.taskService.taskSaved.subscribe(event => {
			switch (event.type) {
			...
			const newTagIds = event.task.tags.filter(id => !this.tags.some(t => t.id === id));
	...
	async ngOnInit(): Promise<void> {
		...
		this.tags = await this.tagService.getTagByIds(this.tasks.flatMap(task => task.tags));
	...
	async loadTasks() {
		...
		if (this.activeClockTime && !this.tasks.some(t => t.id === this.activeClockTime?.task)) {
			const task = await this.taskService.getTaskById(this.activeClockTime?.task);
	...
	setTaskDesctiption() {
		...
		const task = this.tasks.find(t => t.id === this.activeClockTime?.task);
		this.activeTaskDescription = task?.name;
		this.activeTaskTags = this.tags.filter(t => task?.tags.includes(t.id));
	}
